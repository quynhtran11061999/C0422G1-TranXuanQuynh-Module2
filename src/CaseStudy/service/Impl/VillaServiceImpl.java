...
public class VillaServiceImpl extends FacilityServiceImpl {
    ...
    public void add() {
        ...
        }while (!CheckRegex.checkRegexFacilityArea(poolArea));
        int numberFloors;
        while (true) {
            try {
                System.out.print("Số tầng: ");
                numberFloors = Byte.parseByte(scanner.nextLine());
                CheckException.checkInteger(numberFloors);
                break;
            } catch (NumberFormatException | UserException exception) {
                System.err.println(exception.getMessage());
            }
        }
        Villa villa = new Villa(id,serviceName,usableArea,rentalCosts,maxPeople,rentalType,roomStandard,poolArea,numberFloors);
